...
import { validateLocalizedBody, validateParams } from "../../middleware/validate.js";
...
const router = Router();
router.put("/content", validateLocalizedBody(heroContentSchema.partial().required({ id: true })), async (req, res) => {
...
router.put("/stats/:id", validateParams(idParamsSchema), validateLocalizedBody(heroStatSchema.partial()), async (req, res) => {
