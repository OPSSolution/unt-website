...
export const ArticleReaderModal: React.FC<ArticleReaderModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
      <div 
        className="relative w-full max-w-4xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col text-slate-800"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header Bar */}
        <div className="bg-white p-5 border-b border-slate-200 flex items-center justify-between shrink-0 text-slate-900">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-xs text-slate-500 hidden sm:inline">{article.readTime}</span>
          </div>
          <div className="flex items-center space-x-2">
            <button
              onClick={handleShare}
              className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium flex items-center space-x-1.5 transition-colors border border-slate-200"
            >
        ...
        <div className="p-6 sm:p-8 overflow-y-auto space-y-6 flex-1 text-slate-800 text-left">
          ...
          <div className="space-y-4 text-slate-700 text-sm sm:text-base leading-relaxed">
            <p className="text-slate-900 font-semibold text-base sm:text-lg border-l-4 border-emerald-600 pl-4 py-1 bg-emerald-50/50 rounded-r-xl">
              {article.excerpt}
            </p>
          ...
          <div className="pt-4 border-t border-slate-200">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2">Related Topics</span>
            <div className="flex flex-wrap gap-2">
              {article.tags.map((tag, idx) => (
                <span key={idx} className="px-3 py-1 bg-slate-100 text-slate-700 text-xs font-medium rounded-lg border border-slate-200 flex items-center space-x-1">
          ...
          <div className="p-6 rounded-2xl bg-emerald-50 border border-emerald-200 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <h4 className="text-base font-bold text-slate-900">Need Regulatory Assistance or Sourcing Advice?</h4>
              <p className="text-xs text-slate-600 mt-1">Our trade team handles GDCE import filings, HACCP permits & Khmer label registration.</p>
            </div>
            <button
              onClick={() => {
                onClose();
