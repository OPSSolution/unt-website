    ...
    <div className="lg:hidden px-4 pt-2 pb-5 space-y-3 animate-in slide-in-from-top duration-200 text-left">
      ...
      <div className="space-y-1">
        {NAV_LINKS.map((link) => (
          ...
          >
            <span>{link.label}</span><span className="text-xs text-slate-400 dark:text-slate-500">{link.labelKhmer}</span>
          </button>
        ))}
      </div>
      <div className="grid grid-cols-2 gap-2 pt-1">
        <ActionButton icon={Search} label="Search" onClick={onSearch} />
        <ActionButton icon={Calculator} label="Estimator" onClick={onCalculate} />
      </div>
      {setDarkMode && (
