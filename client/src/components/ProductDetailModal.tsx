...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
      <div 
        className="relative w-full max-w-3xl bg-white border border-slate-200 rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col text-slate-800"
      ...
      >
        ...
        <div className="bg-white p-6 border-b border-slate-200 flex items-center justify-between shrink-0 text-slate-900 text-left">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <h3 className="text-lg font-display font-bold text-slate-900 leading-tight">{product.name}</h3>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-800 text-left">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            <div className="relative rounded-2xl overflow-hidden border border-slate-200 bg-slate-100 aspect-video md:aspect-square">
              <img
                src={product.image}
                alt={product.name}
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-2 text-xs">
                <div className="flex justify-between border-b border-slate-200 pb-1.5">
                  <span className="text-slate-500 font-medium">Minimum Order (MOQ):</span>
                  <span className="font-bold text-emerald-700">{product.moq}</span>
                </div>
                <div className="flex justify-between border-b border-slate-200 pb-1.5">
                  <span className="text-slate-500 font-medium">Door-to-Door Lead Time:</span>
                  <span className="font-semibold text-slate-900">{product.leadTime}</span>
                </div>
                {product.shelfLife && (
                  <div className="flex justify-between">
                    <span className="text-slate-500 font-medium">Shelf Life Guarantee:</span>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
            {sampleRequested ? (
              <div className="w-full sm:w-auto p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-xs font-semibold flex items-center space-x-2">
              ...
              <button
                ...
                className="w-full sm:w-auto px-5 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold border border-slate-300 transition-colors"
              >
