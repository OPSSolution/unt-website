...
export function QuoteContactStep({ formData, setFormData, direction }: QuoteStepProps) {
  const fields = [
    { key: 'companyName' as const, type: 'text', placeholder: 'Company / Store Name *', icon: Building2 },
    { key: 'contactName' as const, type: 'text', placeholder: 'Contact Name *', icon: User },
    { key: 'email' as const, type: 'email', placeholder: 'Business Email *', icon: Mail },
    { key: 'phone' as const, type: 'text', placeholder: 'Telegram / Phone *', icon: Phone },
  ];
  ...
  return (
    <div className={`space-y-5 ${stepAnimation(direction)}`}>
      <div>
        ...
        <div className="space-y-3">
          {fields.slice(0, 2).map((field) => (
            <ContactInput key={field.key} {...field} value={formData[field.key]} onChange={(value) => setFormData((current) => ({ ...current, [field.key]: value }))} />
          ))}
      ...
      <div className="flex items-center justify-center gap-5 pt-1 text-[10px] text-slate-400 dark:text-slate-500 font-medium">
        <span className="flex items-center space-x-1.5"><ShieldCheck className="w-3.5 h-3.5 text-emerald-500" /><span>SSL Encrypted</span></span>
        <span className="flex items-center space-x-1.5"><Clock className="w-3.5 h-3.5 text-emerald-500" /><span>24hr Response</span></span>
        <span className="flex items-center space-x-1.5"><BadgeCheck className="w-3.5 h-3.5 text-emerald-500" /><span>No Obligation</span></span>
      </div>
    </div>
  );
}
