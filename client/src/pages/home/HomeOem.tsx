        ...
        <div className="relative rounded-[36px] bg-gradient-to-r from-emerald-950 via-[#0a251c] to-slate-950 p-8 sm:p-12 lg:p-14 text-white overflow-hidden shadow-2xl border border-emerald-500/30">
          ...
          <div className="relative z-20 grid grid-cols-1 lg:grid-cols-12 gap-10 items-center text-left">
            <div className="lg:col-span-7 space-y-6">
              <div className="inline-flex items-center space-x-2 px-3.5 py-1 rounded-full bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 text-xs font-bold uppercase"><Sparkles className="w-3.5 h-3.5 animate-spin" /><span>{c.badge ?? 'OEM & Private Label Excellence'}</span></div>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display font-black"><ScrollTextReveal text={c.heading ?? 'Launch Your Brand with World-Class Formulations'} mode="codepen-title" /></h2>
              <p className="text-emerald-100/90 leading-relaxed">{c.paragraph ?? 'UNT provides end-to-end private label manufacturing with GMP-certified factories in South Korea, Japan, and Vietnam.'}</p>
              ...
              <button onClick={onQuote} className="px-8 py-4 rounded-2xl bg-gradient-to-r from-emerald-400 to-teal-400 text-slate-950 font-black flex items-center gap-3"><span>{c.cta ?? 'Start OEM Private Label Project'}</span><ArrowRight className="w-5 h-5" /></button>
            </div>
