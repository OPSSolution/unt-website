...
export function HomeHeritage() {
  ...
  const content = useHomepageSections().heritage ?? {};
  const { language } = useLanguage();
  const englishOnly = (value: string) => language === 'en' ? value : '';
  const features = FEATURES.map((feature, index) => ({
    ...feature,
    title: content[`feature${index + 1}_title`] ?? englishOnly(feature.title),
  ...
  return (
    <section onMouseMove={handleMouseMove} className="relative py-24 overflow-hidden text-slate-900 dark:text-white border-y border-slate-200/80 dark:border-slate-800/80">
      ...
      <div className="max-w-[1700px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-16 relative z-10"><div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <ScrollReveal animation="left" delay={150}><div className="space-y-6 text-left"><span className="px-3.5 py-1 bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 text-xs font-bold uppercase rounded-full">{content.badge ?? englishOnly('Balancing Heritage with Modern Efficiency')}</span><h2 className="text-3xl sm:text-4xl lg:text-5xl font-display font-bold"><ScrollTextReveal text={content.heading ?? englishOnly('Bridging International Factories with Cambodian Commerce')} mode="codepen-title" /></h2><p className="text-slate-600 dark:text-slate-300">{content.paragraph ?? englishOnly("Global supply chains are complex, but sourcing doesn't have to be. Inspect our operational capabilities below.")}</p><div className="space-y-4">{features.map((feature, index) => { const Icon = feature.icon; const selected = index === activeFeature; return <Card3D key={`${index}-${feature.title}`} intensity={10} onClick={() => setActiveFeature(index)}><button type="button" onMouseEnter={() => setActiveFeature(index)} className={`w-full text-left p-6 rounded-3xl border shadow-xl flex items-start gap-5 transition-all ${selected ? 'bg-white dark:bg-[#0c1322] border-emerald-500 scale-[1.02]' : 'bg-white dark:bg-[#0c1322] border-slate-200 dark:border-slate-800'}`}><div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${selected ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-emerald-600'}`}><Icon className="w-6 h-6" /></div><div><div className="flex flex-wrap justify-between gap-2"><h4 className="font-bold">{feature.title}</h4>{feature.metric && <span className="text-[11px] font-bold text-emerald-600">{feature.metric}</span>}</div><p className="text-xs text-slate-600 dark:text-slate-300 mt-2">{feature.description}</p></div></button></Card3D>; })}</div></div></ScrollReveal>
      </div></div>
    </section>
  );
}
