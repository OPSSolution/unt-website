...
export function HomeSolutions({ onNavigate }: { onNavigate: (tab: PageTab) => void }) {
  ...
  return (
    <section className="w-full max-w-[1700px] mx-auto px-4 sm:px-6 lg:px-12 xl:px-16 relative py-4 sm:py-6">
      ...
      <div className="relative z-10 p-3 sm:p-10 lg:p-14 rounded-3xl sm:rounded-[44px] glass-circle-morphism shadow-2xl overflow-hidden">
        <ScrollReveal animation="up"><div className="text-center space-y-4 mb-8 sm:mb-10"><span className="px-3.5 py-1 bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 text-xs font-bold uppercase rounded-full">{content.badge || (language === 'km' ? 'ហេដ្ឋារចនាសម្ព័ន្ធពាណិជ្ជកម្មពេញលេញ' : 'Full-Spectrum Trading Infrastructure')}</span><h2 className="text-2xl sm:text-4xl lg:text-5xl font-display font-bold text-slate-900 dark:text-white"><ScrollTextReveal key={language} text={content.heading || (language === 'km' ? 'ដំណោះស្រាយរួមបញ្ចូលសម្រាប់ពាណិជ្ជកម្មទំនើប' : 'Integrated Solutions for Modern Commerce')} mode="codepen-title" /></h2><p className="text-slate-600 dark:text-slate-300 text-xs sm:text-base max-w-3xl mx-auto leading-relaxed">{content.subheading || (language === 'km' ? 'Unique Noble Trading Co., Ltd. ផ្តល់ជូនច្រកពាណិជ្ជកម្មពេញលេញ ចាប់ពីការត្រួតពិនិត្យរោងចក្រ រហូតដល់ការចែកចាយក្នុងទីផ្សារ។' : 'Unique Noble Trading Co., Ltd. operates as a complete commercial gateway, managing product supply chains from initial factory audits to local market distribution.')}</p><div className="pt-3 flex justify-center"><div className="inline-flex p-1 rounded-2xl sm:rounded-full bg-slate-100 dark:bg-slate-800 text-xs font-bold max-w-full overflow-x-auto scrollbar-none"><ModeButton active={displayMode === 'grid'} onClick={() => setDisplayMode('grid')} icon={LayoutGrid} label={content.grid_label || (language === 'km' ? 'ទិដ្ឋភាពក្រឡា ៣ ជួរ' : '3-Column Grid View')} /><ModeButton active={displayMode === 'carousel'} onClick={() => setDisplayMode('carousel')} icon={Layers} label={content.carousel_label || (language === 'km' ? 'គ្រាប់រំកិល 3D' : '3D Coverflow Slider')} /></div></div></div></ScrollReveal>
        {displayMode === 'grid' ? <div key={`grid-${language}`} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">{cards.map((card, index) => <ScrollReveal key={`${language}-${index}`} animation="up" delay={(index + 1) * 100}>{card}</ScrollReveal>)}</div> : <CarouselSlider3D key={`carousel-${language}`} autoPlayInterval={6000}>{cards}</CarouselSlider3D>}
      </div>
    </section>
  );
}
