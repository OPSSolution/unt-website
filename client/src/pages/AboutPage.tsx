...
export const AboutPage: React.FC<AboutPageProps> = ({ onOpenQuoteModal }) => {
  ...
  const advIcons = [ShieldCheck, Building2, Truck, Users];
  const hubFlags = [data.hub1_flags ?? '🇹🇭 🇻🇳', data.hub2_flags ?? '🇰🇷 🇯🇵', data.hub3_flags ?? '🇨🇳 🇰🇭'];
  const hubTitles = [
    data.hub1_title ?? 'Bangkok & Ho Chi Minh Corridors',
    data.hub2_title ?? 'Seoul & Tokyo OEM Laboratories',
    data.hub3_title ?? 'Guangzhou & Phnom Penh Central',
  ];
  const hubDescs = [
    data.hub1_desc ?? 'Cross-border overland logistics hub for rapid F&B, organic coconut water, teas, and household consumer product shipments into Cambodia.',
    data.hub2_desc ?? 'Access to audited GMP cosmetics laboratories and health supplement formulators for premium skincare lines and anti-aging treatments.',
    data.hub3_desc ?? 'Bulk manufacturing, custom eco packaging, and central distribution warehouse located in Phnom Penh for rapid order fulfillment.',
  ];
  return (
    <div className="space-y-16 pb-16 animate-fade-in bg-slate-50 dark:bg-[#090D16] text-slate-900 dark:text-slate-100 transition-colors duration-300 bg-ambient-mesh">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center text-left">
        <ScrollReveal animation="right">
          <div className="space-y-6">
            <span className="px-3 py-1 bg-emerald-100 border border-emerald-200 text-emerald-800 text-xs font-bold uppercase tracking-wider rounded-full">
              {data.mission_badge ?? 'Our Purpose & Mission'}
            </span>
            <h2 className="text-3xl sm:text-4xl font-display font-bold text-slate-900 leading-tight">
              {data.mission_heading ?? 'Connecting World-Class Manufacturers with Emerging ASEAN Markets'}
            </h2>
            <p className="text-slate-600 text-sm leading-relaxed">
              {data.mission_p1 ?? 'Founded with the vision of modernizing Cambodian import commerce, UNT Company acts as an essential catalyst for local retailers, pharmacy chains, FMCG distributors, and brand owners. We remove cross-border trade friction by taking full responsibility for supplier auditing, volume pricing negotiation, quality control, customs clearance, and product compliance.'}
            </p>
            <p className="text-slate-600 text-sm leading-relaxed">
              {data.mission_p2 ?? 'Our dual focus on physical distribution and commercial training ensures that our clients not only secure top-tier products, but also possess the negotiation and sales capabilities required to achieve market leadership.'}
            </p>

            <div className="grid grid-cols-2 gap-4 pt-4">
              {[1, 2].map((n) => (
                <div key={n} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
                  <div className="text-3xl font-display font-bold text-emerald-700 dark:text-emerald-400">{data[`stat${n}_value`] ?? (n === 1 ? '100%' : '1,200+')}</div>
                  <div className="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1">{data[`stat${n}_label`] ?? (n === 1 ? 'Regulatory & Tax Audit Compliant' : 'Trade & Sales Professionals Trained')}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <ScrollReveal animation="left" delay={200}>
          ...
          <img
            src={data.mission_image ?? 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?q=80&w=1000&auto=format&fit=crop'}
          ...
          <div className="absolute bottom-6 left-6 right-6 p-6 bg-white/95 dark:bg-slate-800/95 border border-slate-200 dark:border-slate-700 backdrop-blur-md rounded-2xl shadow-lg">
            <div className="text-emerald-700 dark:text-emerald-400 font-bold text-sm">{data.hq_label ?? 'Phnom Penh Corporate Headquarters'}</div>
            <div className="text-slate-600 dark:text-slate-300 text-xs mt-0.5">{data.hq_address ?? 'Phnom Penh Tower, Monivong Blvd, Doun Penh, Phnom Penh'}</div>
          </div>
          </div>
        </ScrollReveal>
      </section>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
            <div className="text-center space-y-3">
              <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs font-bold uppercase tracking-wider rounded-full">
                {data.adv_badge ?? 'Why Business Leaders Choose UNT'}
              </span>
              <h2 className="text-3xl font-display font-bold text-slate-900 dark:text-white">{data.adv_heading ?? 'The UNT Advantage'}</h2>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
              {[1, 2, 3, 4].map((n) => {
                const Icon = advIcons[n - 1];
                return (
                  <Card3D key={n} intensity={12}>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="live-pulse-badge">
                            ...
                            <span>{['ISO Audit', 'GDCE Broker', 'Reefer Logistics', 'Academy Live'][n - 1]}</span>
                          </div>
                        </div>
                        <h3 className="text-lg font-display font-bold text-slate-900 dark:text-white">
                          {data[`adv${n}_title`] ?? ['Direct Factory Audit', 'In-House Customs Brokerage', 'Cold Chain & Logistics', 'Sales & Capacity Building'][n - 1]}
                        </h3>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <ScrollReveal animation="up">
          <div className="text-center space-y-3 max-w-2xl mx-auto">
            <span className="px-3 py-1 bg-emerald-100 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs font-bold uppercase tracking-wider rounded-full">
              {data.net_badge ?? 'Strategic Infrastructure'}
            </span>
            <h2 className="text-3xl font-display font-bold text-slate-900 dark:text-white">
              {data.net_heading ?? 'Our Global Network & Operations Hubs'}
            </h2>
            <p className="text-slate-600 dark:text-slate-300 text-sm">
              {data.net_sub ?? 'Local presence in key manufacturing capitals ensures rapid sampling, immediate factory audits, and direct supplier oversight.'}
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left pt-6">
            {[1, 2, 3].map((n) => (
              <div key={n} className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md space-y-3">
                <div className="text-2xl">{hubFlags[n - 1]}</div>
                <h3 className="text-lg font-display font-bold text-slate-900 dark:text-white">
                  {hubTitles[n - 1]}
                </h3>
          ...
          <div className="pt-6 text-center">
            ...
            >
              <span>{data.cta ?? 'Partner with UNT Company'}</span>
